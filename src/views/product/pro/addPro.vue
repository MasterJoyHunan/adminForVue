        ...
        buildChild() {
            ...
            if (this.chooseSku1.length > 0 && this.chooseSku2.length == 0) {
                this.chooseSku1.forEach((item, index) => {
                    this.formValue.sku.push({ name: item.name, sku_id_1: item.id, sku_id_2: 0, stock: 0, market_price: 0, price: 0, sales_volume: 0 })
                })
            } else if (this.chooseSku1.length > 0 && this.chooseSku2.length > 0) {
                this.chooseSku1.forEach((item, index) => {
                    this.chooseSku2.forEach((item2, index2) => {
                        this.formValue.sku.push({ name: item.name + item2.name, sku_id_1: item.id, sku_id_2: item2.id, stock: 0, market_price: 0, price: 0, sales_volume: 0 })
                    })
                })
            } else if (this.chooseSku1.length == 0 && this.chooseSku2.length > 0) {
                this.chooseSku2.forEach((item, index) => {
                    this.formValue.sku.push({ name: item.name, sku_id_1: 0, sku_id_2: item.id, stock: 0, market_price: 0, price: 0, sales_volume: 0 })
                })
            }
        }
    },
